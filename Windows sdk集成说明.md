...
#### 获取会话列表
	...
	chatManager.getConversations().map((item) => {
			var msgObj = {};
			var unReadMsgMsgId = [];
...
#### 获取历史消息
	...
	chatManager.fetchHistoryMessages(conversationId, type, error, pageSize, startMsgId).result();
#### 插入消息
...
#### 会话操作
	...
	// 会话扩展属性
	conversation.setExtField(strAttr);
