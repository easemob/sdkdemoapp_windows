	...
	if (bRet)
	{
		...
		while (bRet)
		{
			...
			if (lpFaceInfo != NULL)
			{
				...
				lpFaceInfo->m_strEmoticon = xmlSubNode.GetAttribute(_T("emoticon"));
				std::wstring sFile = xmlSubNode.GetAttribute(_T("file"));
				utils::Replace(sFile, _T("/"), _T("\\"));
				lpFaceInfo->m_strFileName = CPaintManagerUI::GetInstancePath() + _T("skin\\EMRes\\") + sFile.c_str();
				tstring strIndex = ZYM::CPath::GetFileNameWithoutExtension(lpFaceInfo->m_strFileName.c_str());
				if (utils::IsDigit(strIndex.c_str()))
...
CFaceInfo * CFaceList::GetFaceInfoByEmoticon(LPCTSTR lpEmoticon)
{
	for (int i = 0; i < (int)m_arrFaceInfo.size(); i++)
	{
		CFaceInfo * lpFaceInfo = m_arrFaceInfo[i];
		if (lpFaceInfo != NULL && lpFaceInfo->m_strEmoticon == lpEmoticon)
			return lpFaceInfo;
	}
