...
void CMenuWnd::Init(CMenuElementUI* pOwner, STRINGorID xml, LPCTSTR pSkinType, POINT point, CControlUI * pParentCtrl/* = NULL*/)
{
	...
	RECT rcWnd = {0};
	Create((m_pOwner == NULL) ? m_hParent : m_pOwner->GetManager()->GetPaintWindow(), NULL, WS_POPUP, WS_EX_TOOLWINDOW | WS_EX_TOPMOST, rcWnd);
...
LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if( uMsg == WM_CREATE ) {
		if( m_pOwner != NULL) {
			...
			rcWork.left = oMonitor.rcWork.left;
			rcWork.right = oMonitor.rcWork.right;
			rcWork.top = oMonitor.rcWork.top;
			rcWork.bottom = oMonitor.rcWork.bottom;
#else
...
void CMenuElementUI::CreateMenuWnd()
{
	...
	POINT pt = {0};
	m_pWindow->Init(static_cast<CMenuElementUI*>(this), _T(""), _T(""), pt, m_pParent);
}
