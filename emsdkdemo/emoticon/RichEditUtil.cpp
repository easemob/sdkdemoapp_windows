...
// 获取文本
void RichEdit_GetText(HWND hWnd, tstring& strText, CFaceList &fl)
{
	REOBJECT reobject;
	LONG nFaceId, nPos = 0;
		...
		if (SUCCEEDED(hr))
		{
			...
			if (CLSID_ImageOle == reobject.clsid)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					if (nFaceId != -1)
					{
						for (int i = 0; i < (int)fl.m_arrFaceInfo.size(); i++)
						{
							CFaceInfo * lpFaceInfo = fl.m_arrFaceInfo[i];
							if (lpFaceInfo != NULL && lpFaceInfo->m_nId == nFaceId)
							{
								TCHAR cBuf[32] = { 0 };
								wsprintf(cBuf, _T("%s"), lpFaceInfo->m_strEmoticon.c_str());
								strText += cBuf;
							}
						}
					}
...
// 获取文本
void RichEdit_GetText(ITextServices * pTextServices, tstring& strText, CFaceList &fl)
{
	REOBJECT reobject;
	LONG nFaceId, nPos = 0;
	tstring strOrgText, strTemp;
