...
CFriendsUI::CFriendsUI(CPaintManagerUI& paint_manager, const CDuiString& sItemDescFile)
	: root_node_(NULL)
	, delay_deltaY_(0)
	, delay_number_(0)
	, delay_left_(0)
...
Node* CFriendsUI::AddNode(const ListItemInfo& item, Node* parent)
{
	if (!parent)
		parent = root_node_;
    ...
    if( !m_dlgBuilder.GetMarkup()->IsValid() ) {
        pListElement = static_cast<CListContainerElementUI*>(m_dlgBuilder.Create(STRINGorID(sItemDescFile_.GetData()), (UINT)0, NULL, &paint_manager_));
    }
    else {
